...
def find_route(grid, total, states, bests):
        ...
        if v.isalpha() and ((k[0], k[1] - 1) not in grid or grid[k[0], k[1] - 1] == '.'):
            ...
            for trial, steps in sorted(possibles.items(), key=lambda x: x[0][1] < 2):
                ...
                if column and trial == max(column):
                    ...
                    trial_grid[trial] = v
                trial_grid[k] = '.'
                find_route({ka: va for ka, va in trial_grid.items()}, steps, states, bests)
...
if __name__ == "__main__":
    with open("test.txt", 'r') as file:
        ...
        grid = {(x, y): data[y][x] for x in range(len(data[0])) for y in range(len(data)) if
                (data[y][x] == '.' or data[y][x].isalpha())}
        vald = [1, 2, 4, 6, 8, 10, 11]
        bests = []
        states = {}
        find_route({k: v for k, v in grid.items()}, 0, states, bests)
        print(min(bests))
        bests = []
        states = {}
